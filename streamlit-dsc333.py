...
st.set_page_config(page_title="Measuring Foot Traffic at Lansing Building Products", layout="wide")
...
st.session_state.current_slide = st.sidebar.radio("Go to Slide", slides, index = slides.index(st.session_state.current_slide))
...
def slide_1():
    ...
    st.subheader("Throughout a work day customers in at all times through 7am to 430pm. One of our main goals was to find " \
    "out when the busiest times of the day were.")

def slide_2():
    st.header("What Days of the week are the busiest?")
    st.subheader("We also wanted to find out which days of the week were the busiest. This would help us know when to have more staff on hand and when to expect more customers.")

def slide_3():
    st.header("Does the Weather Affect Foot Traffic?")
    st.subheader("We wanted to find out if the weather had any effect on foot traffic. We looked at both temperature and precipitation to see if there was any correlation.")

def slide_4():
    st.header("Does the Amount of Money Made in a Day Affect Foot Traffic?")
    st.subheader("We also wanted to find out if there was any correlation between the amount of money made in a day and foot traffic." \
